...
def setup_for_distributed(is_master):
    """
    This function disables printing when not in master process (rank 0)
    """
...
def init_distributed_mode(args):
    ...
    logger.info(f'| distributed init (rank {args.rank} / {args.world_size})')
            ...
            if i % print_freq == 0 or i == len(iterable) - 1:
                ...
                if torch.cuda.is_available():
                    LOG(log_msg.format(
                        ...
                        memory=torch.cuda.max_memory_allocated() / MB), args=self.args)
                else:
                    LOG(log_msg.format(
                        i, len(iterable), eta=eta_string,
                        meters=str(self),
                        time=str(iter_time), data=str(data_time)), args=self.args)
            i += 1
            end = time.time()
